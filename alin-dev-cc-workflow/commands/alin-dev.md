...
- User selects "Yes, fast-path" → Continue to fast-path execution (行 131)
- User selects "No, standard workflow" → Switch to Phase 0 (repository scan)
...
- **If input > 500 characters**: Use summarization + confirmation workflow
  ...
  **Your original input:**
  [显示前 150 字符...] _(truncated for brevity)_

  **Our understanding (summary):**
  [系统生成的摘要，约 100-200 字符，一句话描述核心功能]
  ...
  **Step 4: Response Handling**
  - "Yes, accurate" → Continue to Requirements Gathering (行 275)
  - "Partially correct" → Enter text-based Q&A to clarify differences, then continue
  - "No, let me rephrase" → Prompt user for clearer input, restart Phase 1 with new input
...
Reason: [具体理由，如 "Contains business logic changes requiring verification" 或 "Documentation-only change with minimal risk"]
```
...
- User selects "Yes, create tests" → Execute alin-testing sub-agent
- User selects "No, skip tests" → Complete workflow without testing
...
14. Execute implementation:
    ...
    - If Codex route → alin-codex (Skill delegation to GPT-5 Codex)
