...
- `--force-cc`: Force Claude Code implementation even if Codex-CLI recommended (use with caution for complex tasks)
- `--force-codex`: Force Codex-CLI MCP implementation even for trivial changes (slower but potentially higher quality)
...
- **Size Constraint**: Input description ≤ 30 words
...
```
Input Analysis:
  ├─ Word count > 30? → STANDARD
  ├─ Contains complexity keywords? → STANDARD
  ├─ Contains small-task keywords AND simple scope? → SMALL (with confirmation)
  └─ Uncertain? → STANDARD (default safe)
```
...
Detected pattern: [typo/doc/config/minor-refactor]
Recommended: Fast-path implementation (skip scanning + requirements analysis)
...
Use fast-path workflow?
- yes = Skip Phase 0 & 1, go directly to implementation
- no = Use standard workflow with full analysis
```

**Step 2: User Confirmation Gate**
Wait for explicit user response:
- "yes" / "y" / "fast" / "确认" → Continue fast-path
- "no" / "n" / "standard" / "完整" → Switch to standard workflow (Phase 0)
- No clear response → Default to standard workflow (safety first)
...
alin-code MUST monitor during implementation:
  - If files modified > 3 → Abort fast-path, escalate to standard
  - If lines changed > 50 → Abort fast-path, escalate to standard
  - If unexpected complexity detected (DB changes, API modifications, auth logic) → Abort, escalate

Escalation message:
"⚠️ Fast-path limits exceeded. This task requires full analysis.
Switching to standard workflow with repository scanning..."
→ Restart from Phase 0 with full repository scan
```
...
→ Continue to Phase 0 (Repository Context) following normal workflow
...
- "Which database should we use?" → [PostgreSQL, MySQL, MongoDB, SQLite]
- "What authentication method?" → [JWT, OAuth 2.0, API Keys, Session-based]
- "Include advanced analytics?" → [Yes - full featured, No - basic only, Later phase]
- "Which testing approach?" → [Unit tests only, Integration tests, E2E tests, All three]
...
Clarification Round 1: Structured questions for discrete choices
  → Use AskUserQuestion for technology, approach, scope decisions
  → Example: Database choice, auth method, feature set

Clarification Round 2: Text questions for open-ended topics
  → Use traditional Q&A for business logic, workflows, edge cases
  → Example: "Describe the validation rules", "Explain user workflow"

Clarification Round 3: Final structured confirmation
  → Use AskUserQuestion for final approval or preference decisions
  → Example: Testing level, deployment approach
```
...
AskUserQuestion({
  questions: [{
    ...
    options: [
      {
        label: "Yes, proceed",
        description: "Start Phase 2 implementation chain (alin-generate → alin-code → alin-review)"
      },
      {
        label: "No, refine",
...
**Route to Codex-CLI MCP - if ANY condition met:**
- Total changes ≥20 lines
...
- `--force-codex`: Skip analysis, force Codex-CLI path (even if CC sufficient)
...
AskUserQuestion({
  questions: [{
    ...
    options: [
      {
        label: "Codex-CLI MCP (Recommended)",
        description: "Higher quality code generation, better edge case handling. Uses GPT-5 Codex with approval-on-failure safety. Best for logic changes."
      },
      {
...
**Step 2b: Codex-CLI MCP Implementation Path**
...
- Gate: If score ≥90%, proceed to Step 4; else iterate back to Step 2 with feedback
...
### Phase Transitions
0. Start → Phase -1: Classify task (if --auto-skip enabled)
1. Phase -1 → Fast-Path: If SMALL task and user confirms
   Phase -1 → Phase 0: If STANDARD task, user declines, or classification uncertain
2. Phase 0 → Phase 1: Automatic after scan completes (unless fast-path active)
3. Phase 1 → Approval Gate: Automatic when quality ≥ 90 points
4. Approval Gate → Phase 2: ONLY with explicit user confirmation (via AskUserQuestion)
5. Approval Gate → Phase 1: If user requests refinement
6. Fast-Path → Complete: Direct completion for simple tasks (skip Phase 2 complexity)
...
2. IF --auto-skip enabled: Classify task (Phase -1)
   - SMALL task + user confirms → Fast-path (skip to step 16)
   - STANDARD or user declines → Continue to step 3
...
9. 🛑 STOP and use AskUserQuestion for implementation approval
10. Wait for user response
11. If approved: Start Phase 2 implementation
12. Generate spec with alin-generate
13. 🔀 Implementation Routing Decision (Codex-First):
...
14. Execute implementation:
    - If CC route → alin-code (direct CC implementation)
    - If Codex route → alin-codex (MCP delegation to GPT-5 Codex)
...
- **Codex-First routing (Phase 2 Step 1.5)** - Logic changes default to Codex-CLI MCP for higher quality
- Implementation routing - Analyzes spec complexity: <20 lines trivial→CC, logic/complex→Codex, borderline→ask user
- Force routing options - Use --force-cc or --force-codex to override automatic routing decision
- MCP availability - First run checks if Codex-CLI available; falls back to CC if not
