...
def _auto_save_session(data: dict):
    ...
    # GUARD: completed sessions are immutable records — never auto-overwrite them.
    # This prevents a new feature's state transitions (which use the old feature_id
    # while set-mode hasn't been called yet) from corrupting a finished session.
    ...
    desc = (data.get("requirements", {}).get("user_description") or "").strip()
    derived = (desc[:60] + "…" if len(desc) > 60 else desc) if desc else feature_id
    if not existing_name or existing_name == feature_id:
        name = derived        # placeholder → upgrade to real description
    else:
        name = existing_name  # keep whatever was set deliberately
    ...
    # Sync only THIS feature's artifact subdirectory into the session snapshot.
    # Each artifact type now lives under .fullstack/<artifact>/<feature_id>/ so
    # different features never share a directory and cannot overwrite each other.
    for artifact in ("requirements", "contracts", "analysis", "tasks", "tests"):
        src = Path(WORKSPACE_DIR) / artifact / feature_id
        if src.exists():
...
def cmd_set_feature_name(args):
    """Set display name and optional English slug for the current feature.

    --slug renames the session directory and updates current_feature.id so the
    folder name is readable (e.g. feat-works-submission) instead of feat-ab3c9d12.
    """
    import re
    import shutil
    data = load_workspace()
    ...
    if not display_name:
        die("Feature name cannot be empty")

    # Store display name for session picker label
    data.setdefault("requirements", {})["user_description"] = display_name

    # If an English slug is provided, rename the session/tests directories and update the ID
    slug = (getattr(args, "slug", None) or "").strip()
    if slug:
        # Sanitise: lowercase, alphanumeric + hyphens, max 50 chars
        slug_clean = re.sub(r"[^a-z0-9]+", "-", slug.lower()).strip("-")[:50]
        new_feat_id = f"feat-{slug_clean}"
        old_feat_id = (data.get("current_feature") or {}).get("id", "")

        if old_feat_id and old_feat_id != new_feat_id:
            # Rename .fullstack/sessions/<old> → <new>
            old_sess = Path(SESSIONS_DIR) / old_feat_id
            new_sess = Path(SESSIONS_DIR) / new_feat_id
            if old_sess.exists() and not new_sess.exists():
                old_sess.rename(new_sess)

            # Rename .fullstack/tests/<old> → <new> if it was already created
            old_tests = Path(WORKSPACE_DIR) / "tests" / old_feat_id
            new_tests = Path(WORKSPACE_DIR) / "tests" / new_feat_id
            if old_tests.exists() and not new_tests.exists():
                old_tests.rename(new_tests)

            data["current_feature"]["id"] = new_feat_id
            print(f"renamed: {old_feat_id} → {new_feat_id}")

        print(f"feature_id: {new_feat_id}")

    save_workspace(data)
    _auto_save_session(data)
    print(f"feature_name: {display_name}")
...
def cmd_session_save(args):
    ...
    # Copy only THIS feature's artifact subdirectories from .fullstack/<artifact>/<feature_id>/
    feature_id = data.get("current_feature", {}).get("id", "") or session_id
    for artifact_dir in ("requirements", "contracts", "analysis", "tasks", "tests"):
        src = Path(WORKSPACE_DIR) / artifact_dir / feature_id
        if src.exists():
...
def cmd_session_restore(args):
    ...
    # Restore artifacts into the feature-scoped subdirectory so they don't
    # overwrite another feature's files in the shared .fullstack/ workspace.
    feature_id = (snapshot.get("current_feature") or {}).get("id") or args.session_id
    for artifact_dir in ("requirements", "contracts", "analysis", "tasks", "tests"):
        src = session_dir / artifact_dir
        if src.exists():
            dst = Path(WORKSPACE_DIR) / artifact_dir / feature_id
...
def main():
    ...
    p_sfn = sub.add_parser("set-feature-name")
    p_sfn.add_argument("name")
    p_sfn.add_argument("--slug", default="", help="English kebab-case slug used as folder name")
    ...
    dispatch = {
        ...
        "set-feature-name": cmd_set_feature_name,
